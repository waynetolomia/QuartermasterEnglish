...
function getStudentRank(xp, dept) {
  ...
  if (xp < 800) return isEngine ? "Wiper" : "Ordinary Seaman";
  if (xp < 1500) return isEngine ? "Oiler" : "Able Seaman";
  if (xp < 2500) return isEngine ? "Motorman" : "Bosun";
  if (xp < 4000) return isEngine ? "Third Engineer" : "Third Officer";
  if (xp < 6000) return isEngine ? "Second Engineer" : "Second Officer";
  if (xp < 10000) return isEngine ? "First Engineer" : "Chief Officer";
  return isEngine ? "Chief Engineer" : "Master";
}
...
function renderVocabChallenge() {
  ...
  const cardsHtml = dailyWords.map((word, index) => {
    ...
    return `
      <div class="vocab-card" style="border-color: ${theme.hex}; box-shadow: 0 0 20px ${theme.shadow};">
        <div class="vocab-tier" style="color: ${theme.hex}; border-color: ${theme.hex};">${tiers[index]}</div>
        <span class="vocab-category">${word.category}</span>
        <div style="font-size: 10px; color: #34d399; margin-bottom: 10px; text-transform: uppercase; font-weight: bold; letter-spacing: 1px;">${wordLevel} • ${wordXp} XP</div>
        <h2 class="vocab-term">${word.term}</h2>
        <button class="play-audio-btn secondary-btn" data-term="${word.term}" style="border-color: ${theme.hex}; color: ${theme.hex}; padding: 6px 16px; font-size: 12px; border-radius: 20px; width: auto; margin-bottom: 15px; margin-top: -5px; display: inline-flex; align-items: center; justify-content: center; gap: 5px;">🔊 Listen</button>
        <div class="vocab-def-container">
          <p class="vocab-def" style="color: #e2e8f0;">${word.definition}</p>
          <p class="vocab-example">"${word.example}"</p>
          <div style="border-top: 1px solid rgba(255,255,255,0.1); margin-top: auto; padding-top: 15px;">
            <p style="color: #fcd34d; font-weight: bold; margin: 0; font-size: 18px; letter-spacing: 1px;">${word.thai_translation || ''}</p>
            <p style="color: #94a3b8; font-size: 12px; margin: 5px 0 0 0;">${word.thai_meaning || ''}</p>
          </div>
        </div>
      </div>
    `;
  ...
  dailyVocabSection.innerHTML = `
    <h3 style="text-align: center; color: #f8fafc; margin-bottom: 30px; font-size: 32px; text-transform: uppercase; letter-spacing: 3px; font-family: 'Orbitron', sans-serif; text-shadow: 0 0 15px ${theme.shadow};">
      Memorize Vocabulary
    </h3>
    <div style="display: flex; flex-direction: row; gap: 30px; justify-content: center; flex-wrap: wrap; margin-bottom: 40px;">
      ${cardsHtml}
    </div>
    <div style="text-align: center; width: 100%; display: flex; justify-content: center; gap: 20px; flex-wrap: wrap;">
      ...
      <button id="next-card-btn" style="max-width: 350px; font-size: 16px; padding: 18px 30px; border-radius: 12px; text-transform: uppercase; letter-spacing: 2px; background: linear-gradient(135deg, ${theme.hex} 0%, #0072ff 100%); box-shadow: 0 4px 15px ${theme.shadow}; border: 1px solid ${theme.hex};">
        I'm Ready For Quiz
      </button>
    </div>
  `;
